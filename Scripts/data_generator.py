...
def init(
    siglen: int = 5000, filter: bool = False, hcut: int = 56, channel: list = ["ECG I"]
):
...
def read_data():
    ...
    for name in os.listdir(amy_path):
        signals, signal_headers, _ = highlevel.read_edf(
            os.path.join(amy_path, name), ch_names=ecg_channel
        )
    ...
    for name in os.listdir(amyc_path):
        signals, signal_headers, _ = highlevel.read_edf(
            os.path.join(amyc_path, name), ch_names=ecg_channel
        )
    ...
    for name in os.listdir(norm_path):
        signals, signal_headers, _ = highlevel.read_edf(
            os.path.join(norm_path, name), ch_names=ecg_channel
        )
...
def crop(data: list):
    ...
    for record in data:
        ...
        #     parts.append(
        #         np.array(normalize(record[:, i * signal_len : (i + 1) * signal_len]))
        #     )
        parts.append(np.array(normalize(record[:, 0:signal_len])))
    return parts
